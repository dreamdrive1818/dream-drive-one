...
import { bookingInScope } from "../../lib/vehicle-rules";
...
export class BookingController {
  ...
  async get(@Req() req: Request, @Param("id") id: string) {
    ...
    if (isStaff(user) && booking.userId !== user.id && !bookingInScope(user, booking)) {
  ...
  assignDriver(
    @Req() req: Request,
    @Param("id") id: string,
    @Body() body: { driverId?: string; overrideCity?: boolean }
  ) {
    const user = requireRoles(req, "FLEET_OPS", "BRANCH_MANAGER", "CITY_MANAGER", "SUPER_ADMIN");
    return this.bookings.assignDriver(id, body.driverId ?? "", user, {
      overrideCity: Boolean(body.overrideCity),
    });
  }

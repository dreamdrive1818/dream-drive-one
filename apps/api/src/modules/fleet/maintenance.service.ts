...
import { vehicleScopeWhere } from "../../lib/vehicle-rules";
...
export class MaintenanceEngine {
  ...
  jobs(query: { status?: string; vehicleId?: string; workshopId?: string; branchId?: string } = {}, user?: AuthUser) {
    const status = this.parseStatus(query.status, true);
    const vehicleScope = user ? vehicleScopeWhere(user) : {};

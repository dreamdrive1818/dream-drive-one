...
import { assertPartnerContractActive, insuranceCovers } from "./lib/vehicle-rules";
...
export class BookingEngine {
  ...
  async assignDriver(id: string, driverId: string) {
    ...
    const driver = await prisma.driver.findUnique({
      where: { id: driverId },
      include: { branch: true, leaves: true },
    });
    ...
    if (pickup && driver.branch.cityId !== pickup.cityId) {
      throw new BadRequestException("Driver is based in a different city");
    }
  ...
  listAdmin(query: { status?: BookingStatus; q?: string; from?: string; to?: string } = {}) {
    ...
    return prisma.booking.findMany({
      ...
      include: {
        ...
        driverAssignment: { include: { driver: { select: { id: true, fullName: true } } } },

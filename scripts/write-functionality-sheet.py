...
MODULES = [
    ...
    (
        ...
        [
            ("16.01", "Admin app shell", "Admin", "Next.js apps/admin with sidebar, token gate, city/branch switcher, /cities and /staff routes.", "apps/admin/components/Shell.jsx · /cities · /staff", "Done", "P0", "Inspections stay on booking detail."),
    ...
    (
        ...
        "P0",
        "Done",
        "HTML email via Gmail OAuth/app-password when env is set; otherwise console mock. OTP, booking confirm (self-drive after sign), payment receipt, KYC, Leegality invite, 24h reminder, cancel. Admin /notifications with masked log + resend. Retry stores body and backs off 3x. SMS/WhatsApp later.",
        [
            ("20.01", "Internal send", "System", "POST /internal/notify renders {{vars}} as HTML (no <pre> wrap) or branded plaintext.", "POST /internal/notify · notify.service.ts", "Done", "P0", ""),
            ("20.02", "Gmail delivery", "System", "nodemailer Gmail OAuth2 (refresh token) or app password. From DreamDrive <GMAIL_USER>. Mock to console if GMAIL_USER unset.", "notify.service.ts deliver", "Under Review", "P0", "Copy Gmail OAuth from live backend into apps/api/.env for staging send."),
            ("20.03", "OTP email", "Customer", "identity sendOtp → template otp (live HTML). Failures swallowed so login still works in dev. Production omits devCode.", "POST /v1/auth/otp/send", "Done", "P0", "devCode only when NODE_ENV !== production."),
            ("20.04", "Booking confirmation email", "Customer", "HTML template with car/dates/track link. With-driver on CONFIRMED; self-drive only after agreement signed.", "booking.service.ts paymentCaptured / agreementSigned", "Done", "P0", ""),
            ("20.05", "Templates admin", "Admin", "GET list + PUT by immutable key. Admin /notifications Templates tab. SUPER_ADMIN.", "PUT /v1/admin/notification-templates/:key · app/notifications", "Done", "P1", ""),
            ("20.06", "Delivery log", "Admin", "GET last 200 with masked to + redacted OTP. Admin Log tab.", "GET /v1/admin/notifications", "Done", "P1", ""),
            ("20.07", "Retry failed + resend", "System", "Stores subject/body. Worker every 5 min retries 3x (5/15/45 min). SUPER_ADMIN resend. Hourly trip reminder.", "NotifyRetryJob · POST /internal/notify/retry · POST .../resend", "Done", "P1", ""),
            ("20.08", "SMS / WhatsApp", "Customer", "Same template table; channel sms/whatsapp logs skipped until a provider is wired.", "—", "Not Started", "P2", "After-MVP."),
        ],
    ),
    (
        "21 CRM & Leads",
    ...
    (
        ...
        [
            ...
            (
                ...
                "app/notifications: masked log + attempts + SUPER_ADMIN resend. Templates tab locks key on edit. GET/PUT templates + POST resend.",
                "Done",
                "P1",
                "",
            ),
